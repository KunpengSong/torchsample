...
# Copyright (c) 2017
# Source: https://github.com/zhanghang1989/PyTorch-Encoding (MIT)
###########################################################################
...
class EncNet(BaseNet):
    def __init__(self, nclass, backbone, aux=True, se_loss=True, lateral=False, norm_layer=nn.BatchNorm2d, **kwargs):
...
def get_encnet(dataset='pascal_voc', backbone='resnet50', pretrained=False,
    ...
    model = EncNet(datasets_num_class[dataset.lower()], backbone=backbone, root=root, **kwargs)
    if pretrained:
