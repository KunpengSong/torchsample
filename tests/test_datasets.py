...
class TestDatasets(unittest.TestCase):
    ...
    def testSplitDataset(self):
        ...
        splitdataset = dataset.MultiPartitionDataset(listdataset, {'train': 3, 'val': 1})
        ...
        # test fluent api
        splitdataset = listdataset.partition({'train': 3, 'val': 1})
        splitdataset.select('train')
        self.assertEqual(len(splitdataset), 3)
        self.assertEqual(splitdataset[2], 2)
    ...
    def testSplitDataset_fractions(self):
        h = [0, 1, 2, 3]
        listdataset = dataset.ListDataset(elem_list=h)
        splitdataset = dataset.MultiPartitionDataset(listdataset, {'train': 0.75,
                                                          'val': 0.25})
