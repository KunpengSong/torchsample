...
class ModuleTrainer(object):
            ...
            for epoch_idx in range(nb_epoch):
                ...
                for batch_idx in range(nb_batches):
                    ...
                    if self.cuda_devices:
                        input_batch = [ins.cuda(device_id=self.cuda_devices[0]) for ins in input_batch]
                        if has_target:
                            target_batch = [targs.cuda(device_id=self.cuda_devices[0]) for targs in target_batch]
                ...
                if has_validation_data:
                    val_loss = self.evaluate(*val_data, batch_size=batch_size)
                    if self._has_metrics:
    ...
    def train_on_batch(self, 
        ...
        # assume that model has been pre-loaded and distributed to GPU(s) as necessary
        if self.cuda_devices:
            input_batch = [ins.cuda(device_id=self.cuda_devices[0]) for ins in input_batch]
            if has_target:
                target_batch = [targs.cuda(device_id=self.cuda_devices[0]) for targs in target_batch]
    ...
    def predict_loader(self,
        ...
        if len(prediction_list) > 0 and isinstance(prediction_list[0], (tuple,list)):
            ...
            out_list = []
            for out_i in range(nb_out):
                prediction_out_i = [prediction[out_i] for prediction in prediction_list]
                out_list.append(th.cat(prediction_out_i, 0))
            return out_list
    ...
    def predict_on_batch(self, 
        ...
        if self.cuda_devices:
            inputs = [ins.cuda(device_id=self.cuda_devices[0]) for ins in inputs]
    ...
    def evaluate_on_batch(self, inputs, targets):
        self.model.eval()
        if not isinstance(inputs, (list,tuple)):
            inputs = [inputs]
        ...
        # assume that model has been pre-loaded and distributed to GPU(s) as necessary
        if self.cuda_devices and th.cuda.is_available():
            input_batch = [ins.cuda(device_id=self.cuda_devices[0]) for ins in input_batch]
            if has_target:
                target_batch = [targs.cuda(device_id=self.cuda_devices[0]) for targs in target_batch]
